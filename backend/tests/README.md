...
Run with `npm test` (Vitest, one-shot) or `npm run test:watch`. Requests are made against `src/app.js` directly via `supertest` — no port binding, no real Firebase/Surfboard/Gemini calls.
