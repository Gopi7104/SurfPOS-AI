...
class MockMerchantOnboardingApiService extends Mock implements MerchantOnboardingApiService {}

Response<dynamic> _response(Map<String, dynamic> body) {
  return Response(data: body, statusCode: 200, requestOptions: RequestOptions(path: '/whatever'));
}
...
MerchantApplication _application({String? merchantId, String? storeId}) {
  ...
  return MerchantApplication(
    ...
    storeId: storeId,
    applicationStatus: merchantId == null ? ApplicationStatus.applicationInitiated : ApplicationStatus.merchantCreated,
...
void main() {
  ...
  setUp(() {
    ...
    repository = DashboardRepositoryImpl(apiClient: apiClient, merchantApplicationApi: merchantApplicationApi);
  });

  test('returns hasMerchant:false when the caller has no application yet', () async {
  ...
  test('does not fetch a merchant/store profile while merchantId/storeId are not assigned yet', () async {
    when(() => merchantApplicationApi.list()).thenAnswer((_) async => [_application()]);
  ...
  test('fetches and composes the live Merchant and Store profile once ids are assigned', () async {
    when(() => merchantApplicationApi.list())
        .thenAnswer((_) async => [_application(merchantId: 'm-1', storeId: 's-1')]);
    when(() => dio.get('/merchant', options: any(named: 'options'))).thenAnswer(
      (_) async => _response({
        'success': true,
        'data': {
          'merchant': {'id': 'm-1', 'name': 'Blue Wave Surf Shop', 'companyId': '5560360793'},
        },
      }),
    );
    when(() => dio.get('/stores/s-1', options: any(named: 'options'))).thenAnswer(
      (_) async => _response({
        'success': true,
        'data': {
          'store': {'id': 's-1', 'merchantId': 'm-1', 'name': 'Main Street Store', 'status': 'ACTIVE'},
        },
      }),
    );
