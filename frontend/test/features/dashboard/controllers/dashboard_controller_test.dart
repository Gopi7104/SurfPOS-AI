...
void main() {
  test('build() loads the dashboard on first read', () async {
    ...
    final result = await container.read(dashboardControllerProvider(_uidA).future);

    expect(result, expected);
  });

  test('build() resolves to an empty (no-merchant) state when nothing is onboarded yet', () async {
    final container = ProviderContainer(
      overrides: [
        dashboardRepositoryProvider.overrideWithValue(
  ...
  test('refresh() transitions through loading to the refreshed state', () async {
    ...
    addTearDown(container.listen(dashboardControllerProvider(_uidA), (_, __) {}).close);

    await container.read(dashboardControllerProvider(_uidA).future);

    final future = container.read(dashboardControllerProvider(_uidA).notifier).refresh();
    expect(container.read(dashboardControllerProvider(_uidA)).isLoading, isTrue);
  ...
  test('refresh() surfaces a failure as AsyncError without crashing', () async {
    ...
    final container2 = ProviderContainer(
      overrides: [
        dashboardRepositoryProvider.overrideWithValue(
          FakeDashboardRepository(loadDashboard: () => throw Exception('network down')),
        ),
      ],
    );
    ...
    await expectLater(container2.read(dashboardControllerProvider(_uidA).future), throwsException);
    expect(container2.read(dashboardControllerProvider(_uidA)).hasError, isTrue);
  });
  ...
  test('refresh() is a no-op while a refresh is already in flight', () async {
    ...
    final notifier = container.read(dashboardControllerProvider(_uidA).notifier);
  ...
  group('cross-user isolation (see docs/22_DEVELOPMENT_ROADMAP.md)', () {
    test('two different uids never share provider state, even within the same container', () async {
      final container = ProviderContainer(
        overrides: [
          dashboardRepositoryProvider.overrideWithValue(
            FakeDashboardRepository(loadDashboard: () async => testDashboardState(hasMerchant: false)),
      ...
      final resultA = await containerA.read(dashboardControllerProvider(_uidA).future);
      ...
      final resultB = await container.read(dashboardControllerProvider(_uidB).future);
    ...
    test('re-reading the same uid after it was released returns a fresh instance, not stale data', () async {
      var loadCount = 0;
      final container = ProviderContainer(
        overrides: [
          dashboardRepositoryProvider.overrideWithValue(
      ...
      final sub = container.listen(dashboardControllerProvider(_uidA), (_, __) {});
      await container.read(dashboardControllerProvider(_uidA).future);
    ...
    test('10x alternating login cycle between two merchants never leaks state either direction', () async {
      final merchantA = testDashboardState();
      ...
      final containerB = ProviderContainer(
        overrides: [
          dashboardRepositoryProvider.overrideWithValue(
            FakeDashboardRepository(loadDashboard: () async => merchantBNotOnboarded),
          ),
        ],
      );
      ...
      for (var cycle = 0; cycle < 10; cycle++) {
        final resultA = await container.read(dashboardControllerProvider(_uidA).future);
        expect(resultA.hasMerchant, isTrue, reason: 'cycle $cycle: Merchant A must see their own data');
        expect(resultA.merchant?.name, 'Blue Wave Surf Shop', reason: 'cycle $cycle');

        final resultB = await containerB.read(dashboardControllerProvider(_uidB).future);
        expect(resultB.hasMerchant, isFalse, reason: 'cycle $cycle: Merchant B must never see Merchant A\'s data');
